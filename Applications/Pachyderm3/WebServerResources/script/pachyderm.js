...
function toggleText(linkID) {
   var element = document.getElementById(linkID);
   if (element.className.indexOf("expanded") == -1) {
     element = element.innerHTML.replace('Expand '+String.fromCharCode(0x25C0),'Collapse '+String.fromCharCode(0x25BC));
   }
   else {
     element = element.innerHTML.replace('Collapse '+String.fromCharCode(0x25BC),'Expand '+String.fromCharCode(0x25C0));
   }
   document.getElementById(linkID).innerHTML = element;
/* document.getElementById("saveOnly").click(); */   
}
